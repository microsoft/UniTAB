...
from cococapeval.eval_customized import COCOEvalCap
...
class FlickrCaptionEvaluator(object):
    def __init__(
        self,
        flickr_path,
        subset,
        ...
        self.index = []
        self.exp_id = exp_id
        word2ind_roberta_vocab = json.load(open('roberta-base-vocab-withbbox.json','r'))
        self.roberta_vocab = {}
        for key in word2ind_roberta_vocab:
            self.roberta_vocab[word2ind_roberta_vocab[key]] = key
        self.results = None
    ...
    def update(self, predictions):
        if self.exp_id is not None:
            if 'VQAv2caption' in self.exp_id:
            # if True:
                self.index += [str(predictions["original_img_id"][ii])+'_'+str(predictions["sentence_id"][ii]) for ii in range(len(predictions["original_img_id"]))]
            else:
                self.index += predictions["original_img_id"]
        else:
            self.index += predictions["original_img_id"]
        if "pred_seq" in predictions: self.predictions += predictions['pred_seq']
        else: self.predictions += predictions['pred_logits'].argmax(dim=-1)
        self.gts += predictions['caption_gt']

    def synchronize_between_processes(self):
        index = dist.all_gather(self.index)
        self.index = sum(index, [])
        all_predictions = dist.all_gather(self.predictions)
        self.predictions = sum(all_predictions, [])
        all_gts = dist.all_gather(self.gts)
        self.gts = sum(all_gts, [])

    def summarize(self):
        if dist.is_main_process():
            len_texttoken, eos_token = 50265, 2
            score = {}
            gts_dict, textonly_gts_dict = {},{}
            pred_dict, textonly_pred_dict = {},{}
            for ii in range(len(self.gts)):
                index = self.index[ii]
                gt_str, textonly_gt_str, pred_str, textonly_pred_str = [],[],[],[]
                for token in self.gts[ii]:
                    if int(token)<=eos_token: break
                    gt_str.append(self.roberta_vocab[int(token)])
                    if int(token)<len_texttoken:
                        textonly_gt_str.append(self.roberta_vocab[int(token)])
                textonly_gt_str = ''.join(textonly_gt_str).replace(' ','').replace('<','Ġ<').split('Ġ')
                if index in gts_dict:
                    gts_dict[index].append(' '.join(gt_str))
                    textonly_gts_dict[index].append(' '.join(textonly_gt_str))
                else:
                    gts_dict[index] = [' '.join(gt_str)]
                    textonly_gts_dict[index] = [' '.join(textonly_gt_str)]
                for token in self.predictions[ii]:
                    if int(token)<=eos_token: break
                    pred_str.append(self.roberta_vocab[int(token)])
                    if int(token)<len_texttoken:
                        textonly_pred_str.append(self.roberta_vocab[int(token)])
                textonly_pred_str = ''.join(textonly_pred_str).replace(' ','').replace('<','Ġ<').split('Ġ')
                pred_dict[index] = [' '.join(pred_str)]
                textonly_pred_dict[index] = [' '.join(textonly_pred_str)]
            coco_eval = COCOEvalCap(len(pred_dict))
            ## eval textonly
            print('==============\neval textonly')
            coco_eval.evaluate(textonly_gts_dict, textonly_pred_dict)

            ## eval original
            print('==============\neval original')
            coco_eval.evaluate(gts_dict, pred_dict)

            for metric, val in coco_eval.eval.items():
                score[f'{metric}'] = val

            ## if save json prediction
            header = '' # 'test_'
            if self.exp_id is not None:
                exp_id = 'weights/'+self.exp_id.split('/')[-1]
                os.system('mkdir -p %s'%exp_id)
                os.system('mkdir -p %s/results'%exp_id)
                with open('%s/results/%stextonly_pred_dict_%f.json'%(exp_id,header,score['CIDEr']), 'w') as fp:
                    json.dump(textonly_pred_dict, fp)
                with open('%s/results/%spred_dict_%f.json'%(exp_id,header,score['CIDEr']), 'w') as fp:
